...
void updatePosition(Position * newPosition, Position * position, Move move) {
    ...
    if (  leavingBB & position->board.whiteKing ) {
        ...
        if (leavingSquare == str2index("e1") && arrivingSquare == str2index("g1"))
            movePiece(&(newPosition->board), generateMove(str2index("h1"), str2index("f1")));

        if (leavingSquare == str2index("e1") && arrivingSquare == str2index("c1"))
        ...
        if (leavingSquare == str2index("e8") && arrivingSquare == str2index("g8"))
            movePiece(&(newPosition->board), generateMove(str2index("h8"), str2index("f8")));

        if (leavingSquare == str2index("e8") && arrivingSquare == str2index("c8"))
...
void* evaluatePositionThreadFunction(void *arg) {
...
Node idabThreaded(Position * position, int depth, BOOL verbose) {
    ...
    pthread_t threads[MAX_BRANCHING_FACTOR];
    ...
    for (i=0; i<moveCount; i++) {
        ...
        if (pthread_create(&threads[i], NULL, evaluatePositionThreadFunction, &threadInfo[i]) != 0) {
            printf("!");
            fflush(stdout);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "fm9d:v")) != -1) {
        switch (opt) {
...
//        Node node = iterativeDeepeningAlphaBeta(&(game.position), (char) depth, INT32_MIN, INT32_MAX, FALSE);
        Node node = idabThreadedBestFirst(&(game.position), depth, FALSE);
        move = node.move;
    }
